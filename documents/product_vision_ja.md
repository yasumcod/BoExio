...
- `canonical_price` は差分判定専用の正準価格カラムとし、比較ロジックはこの値のみを使う。
- `list_price` と `display_price` が両方取得できる場合、`list_price` を優先し、未取得時のみ `display_price` を使う。
- `canonical_price` は「価格ソース選定 → 税区分正規化 → 丸め」の順で生成する。
...
差分検知の実装をぶらさないため、価格比較は次の共通ルールで行う。

- 正準価格: 比較対象は `canonical_price` のみ。元カラム (`list_price`, `display_price`) は監査用に保持する。
- 価格ソース優先順位: `list_price` > `display_price`。両方欠損なら「比較不可」として `errors` に記録する。
- 税込/税抜: 比較前に同一軸へ統一する。MVPでは税抜（`tax_type=exclusive`）を正とし、税込のみ取得時は税率情報がある場合のみ税抜換算する。税率が不明な場合は換算せず、別軸データとして保持したうえで警告を出す。
- 通貨: 原則として同一通貨間のみ差分比較する。異通貨は自動換算せず、`currency_mismatch` 警告として `errors` と `summary` に件数を出す。
- 小数・丸め: `canonical_price` は比較前に整数円へ正規化する。端数処理は四捨五入（0.5以上切り上げ）で統一し、差額計算も正規化後の整数値で行う。
